...
<script type="text/javascript">    
    ...
    function sum_total(valor, total, sum1, sum2) {
        //alert("The input value has changed. The new value is: " + val);
        //var y = ++ val;
    ...
    function sum_total_edit(impuesto, total, sum1, sum2) {
        //alert("The input value has changed. The new value is: " + val);
        //var y = ++ val;
        var num1 = document.getElementById(sum1).value;
        var num2 = document.getElementById(sum2).value;
        var num3 = document.getElementById(impuesto).value;
        var tot = num3 * num1 * num2;
        var tot = tot.toFixed(2);
    ...
    function tot_ivatotal(valor, value_ivatot){
        
        
        //var tot_iva    = Number(document.getElementById('gran_tota').value);
        var toiva           = Number(document.getElementById('gran_tota').value);
    ...
    // HASTA AQUI LLEGA EL NUEVO SCRIPT INGRESADO
			...
			<ul class="breadcrumb">
                ...
                if(empty($_POST)===false && $_POST['flag1']=="next"){
                   ...
                   $no_pedido   =   $_POST['id_old'];
                    ...
                    $query1 =   mysql_query("SELECT `id_pedido`,`fecha_pedido`,`fecha_despacho`,`id_cliente`,`id_vendedor`,`total`, `total_siniva`, `impuesto` FROM pedidos_cabecera WHERE id_pedido = $no_pedido");
                ...
                if(empty($_POST)===false && $_POST['flag1']=="guardar"){
                        ...
                        $query8 =   mysql_query("UPDATE `pedidos_secundaria` SET `cantidad` = '$cantidad', `producto` = '$des', `costo` = '$cosst', `precio` = '$precio', `id_proveedor` = '$proveedor', `sub_total` = '$total' WHERE CONCAT(`pedidos_secundaria`.`id_control`) = '$control';");
                            }
                        ...
                        <form class="form-horizontal" method="post" >
						  <fieldset>
                            ...
                            <table border="0" style="width:100%">
							  ...
							  <div class="controls">
								<input  type="text" class="input-medium typeahead" id="iva" name="iva" onkeyup="tot_iva(this.value,'total_iva2' );" value="<?php echo "$iva";?>" required >
							  </div>
                              </td>
                     ...
                     <div class="box-content">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
                          ...
                          ?>
							<tr>
								<td class="center">
                                    <?php //echo"CONTROL: $v_control";?> 
                                <input type="hidden" id="<?php echo"$name_control";?>" name="<?php echo"$name_control";?>" value="<?php echo"$v_control";?>">
                                <input type="text" class="input-small typeahead" id="<?php echo "$name_cantidad";?>" name="<?php echo "$name_cantidad";?>" onkeyup="sum_tot(this.value,'total_iva2','<?php echo "$name_precio";?>','<?php echo "$name_subtotal";?>'); recalcular(this.value,'<?php echo "$name_cosst";?>','<?php echo "$name_total_st";?>'); sum_total_edit('<?php echo "$name_impuesto";?>', '<?php echo "$name_total";?>', '<?php echo "$name_cantidad";?>', '<?php echo "$name_precio";?>');" value="<?php echo "$v_cantidad";?>">
                                </td>
								<td class="center">
                                    <textarea class="cleditor2" id="<?php echo "$name_des";?>" name="<?php echo "$name_des";?>" rows="4"><?php echo "$v_des";?></textarea>
                                </td>
                                
                                <td>
                                    <input type="text" class="input-small typeahead" id="<?php echo "$name_cosst";?>" name="<?php echo "$name_cosst";?>" onkeyup="sum_totst(this.value,'<?php echo "$name_cantidad";?>','<?php echo "$name_total_st";?>','<?php echo "$name_total_st2";?>');" value="<?php echo "$v_cosst";?>">
                                </td>
                                <!--  TOTAL COSTO ST -->
                                 <td>
                                    <span id="<?php echo "$name_total_st";?>" name="<?php echo "$name_total_st";?>"><?php echo "$total_st";?></span>
                                    <input type="hidden" id="<?php echo "$name_total_st2";?>" name="<?php echo "$name_total_st2";?>" value="">
                                </td>
                                <!-- fIN TOTAL COSTO ST  -->
                                <td>
                                    <input type="text" class="input-small typeahead" id="<?php echo "$name_precio";?>" name="<?php echo "$name_precio";?>" onkeyup="sum_tot(this.value,'total_iva2','<?php echo "$name_cantidad";?>','<?php echo "$name_subtotal";?>'); sum_total_edit('<?php echo "$name_impuesto";?>', '<?php echo "$name_total";?>', '<?php echo "$name_cantidad";?>', '<?php echo "$name_precio";?>');" value="<?php echo "$v_precio";?>">
                                </td>
                                <td>
                                    <select class="input-medium" id="<?php echo "$name_proveedor";?>" name="<?php echo "$name_proveedor";?>" data-rel="chosen">
                                        <option value="<?php echo"$v_id_proveedor";?>"><?php echo"$v_proveedor";?></option> 
                                           <?php
                                ...
                                <td>
                                    <span id="<?php echo "$name_subtotal";?>" name="<?php echo "$name_subtotal";?>"><?php echo"$subtotal";?></span>
                                    
                                </td>
                                <!--<td>
								        <select class="input-small" id="<?php echo "$name_impuesto";?>" name="<?php echo "$name_impuesto";?>" onchange="sum_total(this.value, '<?php echo "$name_total";?>', '<?php echo "$name_cantidad";?>', '<?php echo "$name_precio";?>');" data-rel="chosen">
                                           <option value="<?php echo"$v_id_impuesto";?>"><?php echo"$v_impuesto";?></option> 
                          ...
                          ?>
                               <tr>
                                ...
                                <td>
                                    <input type="hidden" id="total_iva2" name="total_iva2"  onkeyup="sum_tot(this.value,'total_iva2','<?php echo "$name_cantidad";?>','<?php echo "$name_subtotal";?>');" value="" >
                                    <span id='span_grantotal' name='span_grantotal'><?php echo "$total";?></span>
                                </td>
                                <td>
                            ...
                            ?>
                                ...
                                echo "<h3>Pedido editado con éxito!</h3>";
                            }
                        ?>
					</div>
			<!-- TERMINA: CONTENIDO ************************************************************** -->
