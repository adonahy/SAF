...
$query  =   mysql_query("SELECT `fecha`,`codigo`,`observaciones`,`debe`,`haber`,`saldo` FROM kardex_pt WHERE fecha >= '$del' AND fecha <= '$al' AND codigo = '$codigo'");


while($r=mysql_fetch_array($query)){
    $fecha              = $r['fecha'];
    $codigo_prod        = $r['codigo'];
    $ob                 = $r['observaciones'];
    $debe               = $r['debe'];
    $haber              = $r['haber'];
    $saldo              = $r['saldo'];
    
    $query1     =   mysql_query("SELECT `codigo`,`descripcion` FROM `inventario_central` WHERE codigo = '$codigo_prod'");
    $result1    =   mysql_fetch_array($query1);
    
    $des        =   $result1['descripcion'];
    
    $val=$val . "
                <tbody>
                    <tr>
				        <td class=\"center\"> $fecha </td>
				        <td class=\"center\"> $codigo_prod </td>
				        <td class=\"center\"> $des </td>
				        <td class=\"center\"> $ob </td>
                        <td class=\"center\"> $debe </td>
                        <td class=\"center\"> $haber </td>
						...
						<form class="form-horizontal">
						  <fieldset>
							 ...
							 <div class="control-group">   
							  ...
							  <div class="controls">
								<input type="text" class="input-medium" id="codigo" name="codigo" value="'. $codigo_prod .'"> 
							  </div>
							</div> 
...
$mpdf->Output('Reporte_de_kardex.pdf', 'D');
