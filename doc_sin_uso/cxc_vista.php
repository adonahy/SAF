		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
				...
				<li>
                    ...
                    <i class="icon-angle-right"></i>
                    <a href="#">Vista de abonos a cuentas por cobrar</a>
                </li>
			</ul>
                <?php
                if($security == 'go'){ 
                ...
                if(empty($_POST)===false && $_POST['flag1']=="guardar"){
                    ...
                    $idabono   =   $_POST['id_abono'];
                        ...
                        $query2 =   mysql_query("UPDATE `cxc_abonos` SET `estatus_comision` = '1' WHERE CONCAT(`cxc_abonos`.`id`) = '$idabono'");
                ...
                <h1>Vista de abonos a proveedores</h1>
                    <div class="box-content">
                        <?php
                            if($flag1=="one"){
                                $id         =    $_POST['cod'];
                                $query1     =    mysql_query("SELECT `id_control`,`no_docto`,`serie`,`cliente`,`total`,`saldo`,`archivo`, `descripcion` FROM cxc WHERE id_control = '$id'");
                                ...
                                $query6     =    mysql_query("SELECT `codigo`,`nombre_comercial` FROM `clientes` WHERE codigo = '$cliente'");
                                $r6         =    mysql_fetch_array($query6);
                                $cliente    =    $r6['nombre_comercial'];
                        ?>
						<form class="form-horizontal" method="post" enctype="multipart/form-data">
						  <fieldset>
                            ...
                            <label class=" control-label" for="pro">CLIENTE: </label>
                            <div class="controls">      
				                <input type="text" class="input-medium typeahead" id="pro" name="pro" value="<?php echo $cliente; ?>" >
				            </div>   
                              ...
                              <div class="controls">
								<input type="text" class="input-medium typeahead" id="no_doc_pago" name="no_doc_pago" value="<?php echo $descrip; ?>" disabled>
							  </div>
                            <br>
                                <table class="table table-striped table-bordered">
                                  ...
                                  <?php
                                    $query7 =   mysql_query("SELECT `id`, `control`, `cantidad`, `fecha`, `no_doc`, `forma_pago`, `estatus_comision` FROM `cxc_abonos` WHERE control = '$id'");
                                    while($r7=mysql_fetch_array($query7)){
                                        $cantidad   =   $r7['cantidad'];
                                        $fecha      =   $r7['fecha'];
                                        $no_doc     =   $r7['no_doc'];
                                        $fpago      =   $r7['forma_pago'];
                                        $estatus    =   $r7['estatus_comision'];
                                        $idcxc_a    =   $r7['id'];
						  ...
						  <fieldset>
                                ...
                                <button type="submit" class="btn btn-primary">Pagar Comision</button>
                                <input type="hidden" id="flag1" name="flag1" value="guardar">
                                <input type="hidden" id="id_abono" name="id_abono" value="<?php echo $idcxc_a; ?>">
