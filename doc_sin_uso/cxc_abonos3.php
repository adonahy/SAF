		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
				...
				<li>
                    ...
                    <i class="icon-angle-right"></i>
                    <a href="#">Ingreso de abonos de clientes</a>
                </li>
			</ul>
                <?php
                if($security == 'go'){ 
                ...
                if(empty($_POST)===false && $_POST['flag1']=="guardar"){
                    ...
                    $pedido    =    $_POST['documento'];
                    $fpago     =    $_POST['forma_pago'];
                    $serie     =    $_POST['serie'];
                    $no_doc    =    $_POST['no_doc_pago'];
                    $idcxc     =    $_POST['idcxc'];
                    $documento =    $serie . "-" . $no_doc;
                   ...
                   $query5=    mysql_query("SELECT `no_recibo`,`saldo` FROM cxc_cabecera WHERE no_recibo = '$control'");
                   ...
                   $saldo  =   $saldo - $abono;
                    ...
                    $query3 =   mysql_query("INSERT INTO `cxc_detalle` (`id_cxc_detalle`, `id_cxc`, `no_docto`,`serie`, `v_abono`, `f_pago`, `banco`, `cuenta`, `fecha`) VALUES (NULL, '$idcxc', '$no_doc', '$serie', '$abono', '$fpago', '$banco', '$cuenta' ,'$da')");
                    $query4 =   mysql_query("UPDATE `cxc_cabecera` SET `saldo` = '$saldo' WHERE CONCAT(`cxc_cabecera`.`no_recibo`) = '$control'");
                    ...
                    $query6 =   mysql_query("INSERT INTO `estados_cuenta` (`id`, `fecha`, `no_docto`, `concepto`, `credito`, `debito`, `saldo`, `banco`, `cuenta`) VALUES (NULL, '$da', '$documento', 'pago de pedido: $pedido', '$abono', '0', '$saldo_banco', '$banco', '$cuenta')");
                    
                    if($saldo <= 0){
                        $query2 =   mysql_query("UPDATE `pedidos_cabecera` SET `estatus_cxc` = '1' WHERE CONCAT(`pedidos_cabecera`.`id_pedido`) = '$control'");
                    }
                     $flag1 = "guardar";
                }
                ...
                ?>
                <h1>Ingreso de abonos de clientes</h1>
                    <div class="box-content">
                        <?php
                            if($flag1=="one"){
                                $id =       $_POST['cod'];
                                //$query1=    mysql_query("SELECT `id_control`,`no_docto`,`serie`,`proveedor`,`total`,`saldo` FROM cxp WHERE id_control = '$id'");
                                $queryant = mysql_query("SELECT `id_cxc`,`no_recibo`,`id_cliente`,`estatus` FROM `cxc_cabecera` WHERE no_recibo = '$id'");
                                $rant =       mysql_fetch_array($queryant);
                                $estatus = $rant['estatus'];
                                
                                if($estatus == "0"){
                                    
                                    
                                    $query1 = mysql_query("SELECT `id_cxc`,`no_recibo`,`id_cliente`,`estatus` FROM `cxc_cabecera` WHERE no_recibo = '$id'");
                               ...
                               // $estatus = $r1['estatus'];
                                ...
                                $idcxc  =   $r1['id_cxc'];
                                //$total_fac= $r1['total']; TOCA DEFINIR COMO LO SACARE
                                //$saldo  =   $r1['saldo']; TOCA DEFINIR COMO LO SACARE
                                $proveedor= $r1['id_cliente']; // POR ACA VAN LAS MODIFICACIONES
                                $querya =   mysql_query("SELECT `id_pedido`,`producto`,`total` FROM `pedidos_secundaria` WHERE id_pedido = '$id' ");
                                $qa     =   mysql_fetch_array($querya);
                                
                                $total_fac  =   $qa['total'];
                                
                                
                               $querydes =   mysql_query("SELECT `id_pedido`,`producto`,`total` FROM `pedidos_secundaria` WHERE id_pedido = '$id' ");
                                    $qdes     =   mysql_fetch_array($querydes);
                                     $descrip    =   $qdes['producto'];
                                ...
                                $querys = mysql_query("SELECT `id_cxc`,`no_docto`,`v_abono` FROM `cxc_detalle` WHERE id_cxc = '$idcxc'");
                                    ...
                                    $queryds = mysql_query("SELECT `id_cxc`,`no_recibo`,`descripcion`,`estatus` FROM `cxc_cabecera` WHERE no_recibo = '$id'");
                                $rds =       mysql_fetch_array($queryds);
                                $descrip =    $rds['descripcion'];
						  ...
						  <fieldset>
                            ...
                            <label class=" control-label" for="cantidad_abono">CANTIDAD DE A ABONAR: </label>
                              ...
                              <select id="forma_pago" name="forma_pago" data-rel="chosen">
                                ...
                                <label class="control-label" for="doc">NO. DE DOCUMENTO DE PAGO</label>
                                <div class="controls">
                                    <input type="text" class="input-medium typeahead" id="no_doc_pago" name="no_doc_pago">
                                </div>
                                <BR>
                                <label class="control-label" for="descripcion">DESCRIPCION</label>
                                    <div class="controls">
                                        
                                        <input type="text" id="descripcion" name="descricion" value="<?php echo $descrip; ?>"  disabled>
                                    </div>
							</div>
							...
							<div class="form-actions">
                                ...
                                <input type="hidden" id="idcxc" name="idcxc" value="<?php echo $idcxc; ?>">
							</div>
						  </fieldset>
						</form>   
