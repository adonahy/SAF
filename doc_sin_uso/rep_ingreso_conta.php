...
while($r=mysql_fetch_array($query)){
    $fecha              = $r['fecha'];
    $no_docto           = $r['no_deposito'];
    $concepto           = $r['razon'];
    $valor              = $r['monto'];
    
    $query2             = mysql_query("INSERT INTO `rep_ingreso_bancos` (`id`, `fecha`, `no_deposito`, `monto`, `razon`) VALUES (NULL, '$fecha', '$no_docto', '$valor', '$concepto')");
	...
	<!-- start: Header -->
		...
		<div class="row-fluid">
			...
			<div id="content" class="span10">
                <h1>Reporte de ingreso a bancos</h1>
...
$mpdf->Output('Reporte_de_ingresos.pdf', 'D');
...
$rows = mysql_query('SELECT `fecha`,`no_deposito`,`razon`,`monto` FROM `rep_ingreso_bancos`');
