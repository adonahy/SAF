		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
				...
				<li>
                    ...
                    <i class="icon-angle-right"></i>
                    <a href="#">Ingreso de abonos a cuentas por cobrar</a>
                </li>
			</ul>
                <?php
                if($security == 'go'){ 
                ...
                if(empty($_POST)===false && $_POST['flag1']=="guardar"){
                    ...
                    $query5=    mysql_query("SELECT `id_control`,`saldo` FROM cxc WHERE id_control = '$control'");
                    ...
                    $query3 =   mysql_query("INSERT INTO `cxc_abonos` (`id`, `control`, `cantidad`, `banco`, `cuenta`, `no_doc` , `forma_pago`, `fecha`) VALUES ('NULL', '$control', '$abono', '$banco', '$cuenta', '$docdepos', '$fpago', '$da')");
                    $query4 =   mysql_query("UPDATE `cxc` SET `saldo` = '$saldo' WHERE CONCAT(`cxc`.`id_control`) = '$control'");
                    ...
                    $query6 =   mysql_query("INSERT INTO `estados_cuenta` (`id`, `fecha`, `no_docto`, `concepto`, `credito`, `debito`, `saldo`, `banco`, `cuenta`) VALUES (NULL, '$da', '$documento', 'pago de cuenta por cobrar: $documento', '$abono', '0', '$saldo_banco', '$banco', '$cuenta')");
                    
                    if($saldo <= 0){
                        $query2 =   mysql_query("UPDATE `cxc` SET `estatus` = '1' WHERE CONCAT(`cxc`.`id_control`) = '$control'");
                    }
                     $flag1 = "guardar";
                }
                ...
                ?>
                <h1>Ingreso de abonos a cuentas por cobrar</h1>
                    <div class="box-content">
                        <?php
                            if($flag1=="one"){
                                $id     =       $_POST['cod'];
                                $query1 =    mysql_query("SELECT `id_control`,`no_docto`,`serie`,`cliente`,`descripcion`,`total`,`saldo` FROM cxc WHERE id_control = '$id'");
                                ...
                                $query7     =   mysql_query("SELECT `codigo`,`nombre_comercial` FROM `clientes` WHERE codigo = '$cliente'");
                                $r7         =   mysql_fetch_array($query7);
                                $cliente     =   $r7['nombre_comercial'];
                        ?>
						<form class="form-horizontal" method="post" enctype="multipart/form-data">
						  <fieldset>
                            ...
                            <label class=" control-label" for="cantidad_abono">CANTIDAD DE A ABONAR: </label>
                              ...
                              <div class="controls">
								<input type="text" class="input-xlarge typeahead" id="no_doc_pago" name="no_doc_pago" value="<?php echo $descrip; ?>" disabled>
							  </div>
							</div>
